met_qaqc <- function(realtime_file,
    ...
    p <- a %>% reshape2::melt(., id = "time") %>% ggplot(.) +
      geom_line(aes(time, value)) + labs(title = paste0("SUGG"," Post imputation"))+
    ...
    if(forecast_site == "PRLA"){

      a <- d1 %>% filter(siteID == "PRPO")
      a <- as.data.frame(a)
      ...
      p <- a %>% reshape2::melt(., id = "time") %>% ggplot(.) +
        geom_line(aes(time, value)) + labs(title = paste0("PRLA"," Post imputation"))+
        facet_wrap(~variable, scales = "free_y") +
        theme_classic()
    ...
    a <- d1 %>% filter(siteID == forecast_site)
    a <- as.data.frame(a)
    ...
    p <- a %>% reshape2::melt(., id = "time") %>% ggplot(.) +
      geom_line(aes(time, value)) + labs(title = paste0(forecast_site," Post imputation"))+
      facet_wrap(~variable, scales = "free_y") +
      theme_classic()
    ...
    identifier <- paste(model_name, forecast_site, sep="_")
