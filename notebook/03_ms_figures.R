...
lake_info <- read_csv(file.path(lake_directory,"notebook","LakeTable_15Nov2021.csv"))
...
combined <- readr::read_csv(scores_files, progress = FALSE) %>%
  ...
  mutate(region = NA,
         region = ifelse(siteID %in% c("BARC","SUGG"), "Florida", region),
         region = ifelse(siteID %in% c("CRAM","LIRO"), "Wisconsin", region),
         region = ifelse(siteID %in% c("PRLA","PRPO"), "North Dakota", region),
         region = factor(region, levels = c("North Dakota","Wisconsin","Florida")),
