...
for(j in 1:length(sites)){
  ...
  download_neon_files(siteID = config$location$site_id,
                      ...
                      start_date = NA, # as.Date("2021-01-01"),
                      raw_data_directory = config$file_path$data_directory)

  ##'
  # Download the latest "early release" data from Bobby Hensley at NEON

  message("    Downloading non-portal NEON data")

  prop_neon <- readr::read_csv("https://raw.githubusercontent.com/FLARE-forecast/NEON-proprietary-data/master/surface_sonde_NEON_raw.csv", show_col_types = FALSE)
